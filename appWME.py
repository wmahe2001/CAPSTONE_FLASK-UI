...
def index():
    ...
    stats = {    'most_categories' : top_category['Category'].iloc[0],
                'total': top_category['Jumlah'].loc[0],
                'rev_table' : df2[['Category','App','Reviews','Rating']].\
                sort_values('Reviews', ascending = False).\
                head(10).reset_index().iloc[:,1:].to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])}
    ...
    df2.groupby(by='Category').agg({'Installs' : 'sum'}).sort_values(by='Installs', ascending=False).head(10).reset_index()

    # Dataframe baru dengan urutan aplikasi ter-install terbanyak
    Install_Age = df2.groupby(by='Category').\
    agg({'Installs' : 'sum'}).\
    sort_values(by='Installs', ascending=False).head(10).\
    reset_index()

    # plot bar horizontal
    X = Install_Age['Category']
    Y = Install_Age['Installs']
    ...
    # bagian ini digunakan untuk menyimpan plot dalam format image.png
    plt.savefig('Install_Age.png',bbox_inches="tight") 
